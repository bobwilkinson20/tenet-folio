...
# NOTE: This dict lives in the worker process.  With multiple uvicorn
# workers (--workers N > 1) each worker has its own copy, so an auth
# URL generated by one worker won't be found by another.  This is
# acceptable for the single-worker local development use case.
# ------------------------------------------------------------------
...
def oauth_callback(
    ...
    # Reconstruct the full received URL for schwab-py (URL-encode params)
    received_url = (
        f"{settings.SCHWAB_CALLBACK_URL}"
        f"?{urlencode({'code': code, 'session': session, 'state': state})}"
    )
