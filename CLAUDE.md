...
- `utc_to_local_date(utc_dt)` from `utils.datetime` — when converting a stored UTC timestamp to the local date it represents (e.g., determining which calendar day a sync occurred on)
